...
"""
This is the script for 4-hr LSTM Forecasting Model:
...
For 4-hr forecasting model w/o topographic features, replace => x_cols = ['w_depth','ELV', 'DTW', 'TWI'] with x_cols = ['w_depth']
...
with mlflow.start_run(run_name=trial_all):
    ...
    # Load the model
    model = load_model('best1 LSTM D0_R40_S22 P6 44_MSE_Bayes.h5')
    ...
    for t in range(1, n_ahead+1):
        for event_id in events_to_plot:
            ...
            plt.savefig(f"1.Result/figs_LSTM/{hp_model}_train_t+{t}_Event_{event_id}.png")
            mlflow.log_artifact(f"1.Result/figs_LSTM/{hp_model}_train_t+{t}_Event_{event_id}.png")       
    ...
    #csv    
    train_data_1_inv.to_csv(f"1.Result/stat_LSTM/All_train_{trial_model}_{hp_model}_{trial_data}_{trial_param}_{trial_time}_{trial_loss}_{trial_hp}.csv") 
    mlflow.log_artifact(f"1.Result/stat_LSTM/All_train_{trial_model}_{hp_model}_{trial_data}_{trial_param}_{trial_time}_{trial_loss}_{trial_hp}.csv")     
    ...
    for t in range(1, n_ahead+1):
        for event_id in events_to_plot:
            ...
            fig.suptitle(f't+{t} Event:{event_id}', y=1.05)  
            plt.savefig(f"1.Result/figs_LSTM/{hp_model}_test_t+{t}_Event_{event_id}.png")
            mlflow.log_artifact(f"1.Result/figs_LSTM/{hp_model}_test_t+{t}_Event_{event_id}.png")
    ...
    #csv    
    test_data_1_inv.to_csv(f"1.Result/stat_LSTM/All_test_{trial_model}_{hp_model}_{trial_data}_{trial_param}_{trial_time}_{trial_loss}_{trial_hp}.csv") 
    mlflow.log_artifact(f"1.Result/stat_LSTM/All_test_{trial_model}_{hp_model}_{trial_data}_{trial_param}_{trial_time}_{trial_loss}_{trial_hp}.csv")    
